...
# Builder task prompt template (extracted to avoid rebuilding 100+ lines per task)
BUILDER_PROMPT_TEMPLATE = """You are the Builder agent implementing Task {task_num} of {total_tasks}.
...
class AutonomousOrchestrator:
    ...
    def _run_multi_agent(self) -> Dict:
        ...
        try:
            ...
            print("   This causes builds to fall back to SLOW sequential mode (10x slower)")
            print("   Fix the error above to enable fast parallel execution\n")

            # Check if we should fallback or fail fast
            fallback_enabled = os.getenv('FOUNDRY_FALLBACK_TO_SEQUENTIAL', 'false').lower() in ('true', '1', 'yes')

            if fallback_enabled:
                print("   ⚠️  FOUNDRY_FALLBACK_TO_SEQUENTIAL=true - Falling back to sequential mode...")
                print("   Set FOUNDRY_FALLBACK_TO_SEQUENTIAL=false to fail fast and fix the root cause\n")
                self.use_multi_agent = False
                return self.run()
            else:
                print("   ❌ FOUNDRY_FALLBACK_TO_SEQUENTIAL=false - Failing fast!")
                print("   Set FOUNDRY_FALLBACK_TO_SEQUENTIAL=true to enable fallback (not recommended)\n")
                raise
...
"""
            ...
            # Build previous tasks context (use list comprehension to avoid O(n²) string concat)
            previous_context = ""
            if previous_files_created:
                ...
                for task_num, files in previous_files_created:
                    lines.append(f"Task {task_num} created:")
                    lines.extend(f"  ✓ {file_path}" for file_path in sorted(files))
                lines.extend([
            ...
            # Build task prompt using template (avoids rebuilding 130+ static lines per task)
            task_prompt = BUILDER_PROMPT_TEMPLATE.format(
                task_num=i,
