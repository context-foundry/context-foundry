...
class AutonomousOrchestrator:
    ...
    def __init__(
        ...
        git_memory: bool = False,  # If True, initialize git and commit agent memory
    ):
        ...
        self.git_memory = git_memory  # If True, initialize git and commit agent memory
...
"""
    ...
    def _init_git_if_needed(self):
        ...
        try:
            ...
            if result.returncode == 0:
                console.print("   [green]✓[/green] Git initialized for memory tracking")

                # Create initial commit with just a README
                readme_content = f"# {self.project_name}\n\nGenerated by Context Foundry\n"
                readme_path = self.project_dir / "README.md"
                readme_path.write_text(readme_content)

                subprocess.run(["git", "add", "README.md"], capture_output=True, timeout=10)
                subprocess.run(
                    ["git", "commit", "-m", "feat: Initialize project with Context Foundry"],
                    capture_output=True,
                    timeout=10
                )
            else:
                console.print(f"   [yellow]⚠️  Git init failed: {result.stderr}[/yellow]")

        except Exception as e:
            console.print(f"   [yellow]⚠️  Could not initialize git: {e}[/yellow]")
        finally:
