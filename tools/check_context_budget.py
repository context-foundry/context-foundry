    ...
    if session_path.exists():
        try:
            with open(session_path, "r") as f:
                ...
                context["started_at"] = session.get(
                    "started_at", session.get("created_at", "N/A")
                )
...
def print_build_context_header(context: Dict, width: int = 78):
    ...
    if status == "completed":
        ...
        phase_info = (
            f"Phase {phase_number} ({current_phase})" if phase_number else current_phase
        )
...
Examples:
  ...
  python3 tools/check_context_budget.py --report
    ...
    parser.add_argument(
        "--phase", help="Phase name (scout, architect, builder, test, etc.)"
    )
