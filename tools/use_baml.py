...
def main():
    ...
    phase_parser = subparsers.add_parser(
        "update-phase", help="Update phase tracking with BAML"
    )
    phase_parser.add_argument(
        "phase", help="Phase name (Scout, Architect, Builder, etc.)"
    )
    phase_parser.add_argument(
        "status", help="Phase status (researching, designing, etc.)"
    )
    phase_parser.add_argument("detail", help="Progress detail message")
    phase_parser.add_argument(
        "--session-id", default="context-foundry", help="Session ID"
    )
    phase_parser.add_argument(
        "--iteration", type=int, default=0, help="Test iteration number"
    )

    # Scout report command
    scout_parser = subparsers.add_parser(
        "scout-report", help="Generate Scout report with BAML"
    )
    ...
    scout_parser.add_argument(
        "--patterns", default="", help="Past patterns to consider"
    )

    # Architecture command
    arch_parser = subparsers.add_parser(
        "architecture", help="Generate architecture with BAML"
    )
    ...
    build_parser = subparsers.add_parser(
        "validate-build", help="Validate build result with BAML"
    )
    ...
    # Scout report command
        ...
        if result:
            ...
            print(
                "BAML Scout report generation not available (falling back to orchestrator's method)"
            )
            return 1
    ...
    # Architecture command
        ...
        result = generate_architecture_baml(
            scout_report_json=args.scout_json, flagged_risks=risks
        )
        if result:
            print(json.dumps(result, indent=2))
            return 0
            ...
            print(
                "BAML Architecture generation not available (falling back to orchestrator's method)"
            )
            return 1

    # Validate build result command
    elif args.command == "validate-build":
        result = validate_build_result_baml(result_json=args.result_json)
        if result:
            print(json.dumps(result, indent=2))
            return 0
        else:
            print(
                "BAML build validation not available (falling back to orchestrator's method)"
            )
            return 1
