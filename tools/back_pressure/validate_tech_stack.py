...
def validate_tech_stack(scout_report_path: str) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "errors": [
                {
                    "message": f"Scout report not found: {scout_report_path}",
                    "fix": "Ensure Scout phase completed successfully",
                    "severity": "error",
                }
            ],
            "warnings": [],
    ...
    if not tech_stack:
        ...
        return {
            ...
            "warnings": [
                {
                    "message": "No technology stack section found in scout report - skipping validation"
                }
            ],
            "checks_performed": 0,
            "duration": time.time() - start_time,
...
def extract_tech_stack(scout_report_content: str) -> List[Dict]:
    ...
    for line in tech_section.split("\n"):
        ...
        if match:
            ...
            tech_stack.append(
                {"type": category, "name": name, "version": version, "raw": value}
            )
...
def check_language_available(language: str, min_version: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        return {"status": "warning", "message": f"{language} check failed: {str(e)}"}
