...
def analyze_prompt_structure(
    ...
    if boundary_line is None:
        ...
        for i, line in enumerate(lines):
            if (
                "BEGIN EXECUTION NOW" in line
                or "BEGIN AUTONOMOUS EXECUTION NOW" in line
                or "START NOW" in line
            ):
    ...
    savings_pct = (
        (savings / cost_without_caching) * 100 if cost_without_caching > 0 else 0
    )
    ...
    if static_cacheable:
        recommendations.append(
            f"✅ Static section meets minimum token requirement ({static_tokens:,} > {min_tokens})"
        )
    else:
        recommendations.append(
            f"❌ Static section too small ({static_tokens:,} < {min_tokens}) - caching will not work"
        )

    if boundary_line:
        recommendations.append(f"✅ Cache boundary at line {boundary_line}")
    else:
        recommendations.append("⚠️  No cache boundary marker found - using heuristic")

    if savings_pct > 80:
        recommendations.append(
            f"✅ Expected savings: {savings_pct:.1f}% (50 builds: ${cost_without_caching:.2f} → ${cost_with_caching:.2f})"
        )
    else:
        recommendations.append(
            f"⚠️  Expected savings: {savings_pct:.1f}% (lower than typical 80-90%)"
        )

    if dynamic_tokens < 100:
        recommendations.append(
            f"⚠️  Dynamic section very small ({dynamic_tokens} tokens) - consider expanding"
        )
...
def print_analysis_report(analysis: Dict[str, Any]):
    ...
    print(
        f"   Has boundary marker: {'✅ Yes' if analysis['has_boundary_marker'] else '❌ No'}"
    )
    ...
    print(
        f"   Savings:         ${cost['savings_50_builds']:.4f} ({cost['savings_percentage']:.1f}%)"
    )
