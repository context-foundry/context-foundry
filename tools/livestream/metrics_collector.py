...
class PhaseFileWatcher(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        timer = threading.Timer(
            self.debounce_delay, self._handle_phase_update, args=[file_path]
        )
        timer.start()
...
class MetricsCollector:
            ...
            if watch_path.exists():
                try:
                    self.observer.schedule(
                        self.watcher, str(watch_path), recursive=True
                    )
            ...
            self.db.create_task(
                {
                    ...
                    "working_directory": task.get(
                        "working_directory", task.get("cwd", "")
                    ),
    ...
    async def collect_token_metrics(self, task: Dict[str, Any]):
        ...
        try:
            ...
            try:
                ...
                phase_stats = metrics_db.get_phase_totals(
                    task.get("current_phase", "Unknown"), days=7
                )
            ...
            self.db.add_metric(
                task_id,
                {
                    ...
                    "estimated_remaining_seconds": task.get(
    ...
    async def collect_agent_metrics(self, task: Dict[str, Any]):
        ...
        try:
            ...
            phase_recorded = any(p.get("phase") == current_phase for p in existing)

            if not phase_recorded and current_phase in [
                "Scout",
                "Architect",
                "Builder",
                "Test",
                    ...
                    for issue in feedback.get("issues_found", []):
                        ...
                        self.db.add_decision(
                            task_id,
                            {
                                ...
                                "used_lessons_learned": bool(
                                    issue.get("applies_to_phases")
                                ),
        ...
        try:
            ...
            if common_issues_file.exists():
                ...
                for pattern in patterns_data.get("patterns", []):
                    ...
                    self.db.add_pattern_effectiveness(
                        task_id,
                        {
