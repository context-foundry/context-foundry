...
class ScoutAgent:
        ...
        for py_file in py_files:
            try:
                ...
                for i, line in enumerate(lines, 1):
                    ...
                    if 'TODO' in line_stripped and any(keyword in line_stripped.lower() for keyword in ['add', 'implement', 'feature', 'support']):
                        self.findings.append(Finding(
                    ...
                    if 'def ' in line:
                        ...
                        if func_name and i < len(lines):
                            ...
                            if 'pass' in next_lines and 'NotImplementedError' not in next_lines:
                                if not any(x in next_lines for x in ['try:', 'except:', 'finally:']):
                                    self.findings.append(Finding(
        ...
        for api_file in api_files:
            try:
                ...
                if 'def list' in content.lower() or 'get_all' in content.lower():
                    if 'limit' not in content.lower() and 'offset' not in content.lower() and 'page' not in content.lower():
                        self.findings.append(Finding(
                ...
                has_post = bool(re.search(r'def\s+(post|create)', content, re.IGNORECASE))
                ...
                crud_ops = {'GET': has_get, 'POST': has_post, 'PUT': has_put, 'DELETE': has_delete}
                ...
                if 'app.route' in content or '@router' in content:
                    if 'rate_limit' not in content.lower() and 'ratelimit' not in content.lower() and 'throttle' not in content.lower():
                        self.findings.append(Finding(
    ...
    def _scan_developer_experience(self):
        ...
        for py_file in py_files:
            try:
                ...
                unclear_vars = []
                for match in re.finditer(r'\b(x|y|z|tmp|temp|data|val|var|foo|bar)\s*=', content):
                    line_num = content[:match.start()].count('\n') + 1
                ...
                for i, match in enumerate(func_matches):
                    ...
                    if i + 1 < len(func_matches):
                        ...
                        if next_indent <= func_indent:
                            func_end = content[:func_matches[i + 1].start()].count('\n') + 1
    ...
    def _scan_modern_language_features(self):
        ...
        for py_file in py_files:
            try:
                ...
                if (old_format_count > 3 or format_count > 5) and fstring_count < (old_format_count + format_count) * 0.3:
                    self.findings.append(Finding(
                        title=f"Modernize: Use f-strings in {py_file.name}",
                        finding_type='enhancement',
                        priority='P4',
                        category=['modernization', 'readability'],
                        description=f"Found old-style string formatting (%s, .format()). Consider using f-strings for better readability.",
                        file_path=str(py_file.relative_to(self.project_root)),
                        effort='small'
                    ))
    ...
    def _scan_observability(self):
        ...
        for py_file in py_files:
            try:
                ...
                has_logging = 'logging' in content or 'logger' in content or 'print' in content
                ...
                if py_file.name in ['builder_agent.py', 'architect_agent.py', 'scout_agent.py']:
                    func_count = content.count('def ')
    ...
    def _scan_user_experience(self):
        ...
        # Check CLI files
        cli_files = list(self.project_root.glob("**/cli*.py")) + \
        ...
        for cli_file in cli_files:
            ...
            try:
                ...
                if 'argparse' in content or 'click' in content:
                    ...
                    arg_count = content.count('add_argument') + content.count('@click.option')
                    ...
                    if arg_count > 0 and help_count < arg_count * 0.5:
                        self.findings.append(Finding(
                            ...
                            file_path=str(cli_file.relative_to(self.project_root)) if cli_file.is_relative_to(self.project_root) else cli_file.name,
                            effort='small'
                        ))
                    ...
                    if re.search(pattern, content, re.IGNORECASE):
                        if 'input(' not in content and 'confirm' not in content.lower():
                            self.findings.append(Finding(
                                ...
                                priority='P2',
                                category=['ux', 'safety'],
                                description=f"Found destructive operations (delete/remove) without confirmation prompts. Add user confirmation to prevent accidents.",
                                file_path=str(cli_file.relative_to(self.project_root)) if cli_file.is_relative_to(self.project_root) else cli_file.name,
                                effort='small'
                            ))
                            break
    ...
    def _scan_configuration(self):
        ...
        for py_file in py_files:
            try:
                ...
                hardcoded_patterns = [
                    ...
                    (r'(api_key|password|secret)\s*=\s*["\'](?!env|get)[^"\']+["\']', 'Secret'),
                    (r'/tmp/[\w-]+', 'Path'),
                ...
                if 'os.getenv' in content or 'os.environ' in content:
                    has_validation = 'required' in content.lower() or 'assert' in content or 'raise' in content
        ...
        if (self.project_root / ".env").exists() and not (self.project_root / ".env.example").exists():
            self.findings.append(Finding(
            ...
            try:
                ...
                if 'class ' in content:
                    ...
                    if len(classes) > 2 and not has_abc:
                        ...
                        repeated_methods = [m for m, count in method_counts.items() if count > 2]
                ...
                if py_file.name in ['scout_agent.py', 'builder_agent.py', 'architect_agent.py']:
                    scan_methods = len(re.findall(r'def\s+_scan_\w+', content))

                    if scan_methods > 5:
                        # Check if plugin system exists
                        has_plugin_system = 'plugin' in content.lower() or 'register' in content.lower()
                ...
                import_count = len(re.findall(r'^from\s+[\w.]+\s+import', content, re.MULTILINE))
                if import_count > 15:
                    self.findings.append(Finding(
