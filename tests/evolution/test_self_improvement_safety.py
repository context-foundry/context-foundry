...
class TestProtectedFilesSafety:
    ...
    def test_protected_file_daemon_py(self, mode):
        ...
        assert mode._is_protected_file(
            "/Users/test/context-foundry/tools/evolution/daemon.py"
        )
    ...
    def test_protected_file_self_improvement_py(self, mode):
        ...
        assert mode._is_protected_file(
            "/path/to/tools/evolution/modes/self_improvement.py"
        )
        ...
        with patch("subprocess.Popen") as mock_popen:
            with patch("builtins.open", mock_open()) as mock_file:
                ...
                result = mode._delegate_to_context_foundry(
                    prompt="Test prompt", branch_name="test-branch"
                )
...
class TestTODOPrioritization:
    ...
    def test_fixme_higher_priority_than_todo(self, mode):
        ...
        fixme_priority, fixme_cat = mode._prioritize_todo(
            "# FIXME: Bug in code", "/test/file.py"
        )
        todo_priority, todo_cat = mode._prioritize_todo(
            "# TODO: Add feature", "/test/file.py"
        )
    ...
    def test_security_todos_get_highest_priority(self, mode):
        ...
        security_priority, security_cat = mode._prioritize_todo(
            "# TODO: Fix SQL injection vulnerability", "/test/file.py"
        )

        regular_priority, regular_cat = mode._prioritize_todo(
            "# TODO: Add logging", "/test/file.py"
        )
    ...
    def test_urgent_keywords_increase_priority(self, mode):
        ...
        urgent_priority, _ = mode._prioritize_todo(
            "# TODO: URGENT - fix critical bug", "/test/file.py"
        )
        normal_priority, _ = mode._prioritize_todo(
            "# TODO: Maybe add feature later", "/test/file.py"
        )
    ...
    def test_documentation_todos_get_lower_priority(self, mode):
        ...
        doc_priority, doc_cat = mode._prioritize_todo(
            "# TODO: Add docstring", "/test/file.py"
        )
        feature_priority, feature_cat = mode._prioritize_todo(
            "# TODO: Implement feature", "/test/file.py"
        )
...
class TestTaskValidation:
    ...
    def test_validate_result_success(self, mode):
        ...
        result = TaskResult(
            success=True, output={"status": "claude_spawned", "pid": 123}
        )
    ...
    def test_validate_result_failure(self, mode):
        ...
        result = TaskResult(success=False, output=None, error="Test error")
...
class TestExecuteTaskErrorPaths:
    ...
    def test_execute_task_with_unknown_action(self, mode):
        ...
        task = Task(
            ...
            status="pending",
            priority=5,
            params={"action": "unknown_action_type", "description": "Some improvement"},
            created_at=datetime.utcnow().isoformat(),
        )
    ...
    def test_execute_task_handles_exceptions(self, mode):
        ...
        with patch.object(
            mode, "_delegate_to_context_foundry", side_effect=Exception("Test error")
        ):
            result = mode.execute_task(task)
...
class TestFindTodosConfigLoading:
    ...
    def test_find_todos_with_configured_directories(self, mode):
        ...
        with patch.object(
            mode, "_load_search_config", return_value=["custom/dir1", "custom/dir2"]
        ):
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout="custom/dir1/file.py:10:# TODO: Test task\n"
                )
    ...
    def test_find_todos_with_no_config_uses_defaults(self, mode):
        ...
        with patch.object(mode, "_load_search_config", return_value=None):
            with patch("subprocess.run") as mock_run:
                mock_run.return_value = Mock(
                    returncode=0, stdout="tools/cache/file.py:10:# TODO: Test task\n"
                )
...
class TestPrioritizeTodoEdgeCases:
    ...
    def test_prioritize_todo_with_performance_keyword(self, mode):
        ...
        priority, category = mode._prioritize_todo(
            "# TODO: Optimize performance bottleneck in cache lookup", "/test/file.py"
        )
    ...
    def test_prioritize_todo_with_bug_in_protected_file(self, mode):
        ...
        priority, category = mode._prioritize_todo(
            "# FIXME: Critical bug in task assignment", "/tools/evolution/daemon.py"
        )
    ...
    def test_prioritize_todo_with_evolution_system_file(self, mode):
        ...
        priority, category = mode._prioritize_todo(
            "# TODO: Improve task queue performance", "/tools/evolution/task_queue.py"
        )
    ...
    def test_prioritize_todo_with_test_coverage_keyword(self, mode):
        ...
        priority, category = mode._prioritize_todo(
            "# TODO: Add test coverage for edge cases", "/tools/cache/cache_manager.py"
        )
