...
class TestGitHubIntegration:
    ...
    def test_get_github_owner_from_https_url(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = Mock(
                returncode=0,
                stdout='https://github.com/snedea/context-foundry.git\n'
            )
    ...
    def test_get_github_owner_from_ssh_url(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = Mock(
                returncode=0,
                stdout='git@github.com:snedea/context-foundry.git\n'
            )
...
class TestDatabaseQueries:
    ...
    def test_get_daemon_status_running(self, temp_db):
        ...
        with patch('tools.evolution.communication.web_dashboard_server.get_db_path', return_value=temp_db):
    ...
    def test_get_daemon_status_no_db(self):
        ...
        with patch('tools.evolution.communication.web_dashboard_server.get_db_path', return_value='/nonexistent/db.db'):
            status = web_dashboard_server.get_daemon_status()
    ...
    def test_get_task_stats(self, temp_db):
        ...
        with patch('tools.evolution.communication.web_dashboard_server.get_db_path', return_value=Path(temp_db)):
            stats = web_dashboard_server.get_task_stats()

            assert isinstance(stats, dict)
            # Should have counts for different statuses
            assert 'completed' in str(stats) or 'pending' in str(stats) or isinstance(stats.get('total'), int)
    ...
    def test_get_recent_tasks(self, temp_db):
        ...
        c.execute("INSERT INTO tasks VALUES ('1', 'self_improvement', 'completed', 5, '2025-01-19 00:00:00')")
        c.execute("INSERT INTO tasks VALUES ('2', 'self_improvement', 'running', 5, '2025-01-19 01:00:00')")
        conn.commit()
        conn.close()
