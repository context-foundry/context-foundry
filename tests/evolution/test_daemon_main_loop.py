...
class TestDaemonMainLoop:
            ...
            with patch.object(daemon, '_interruptible_sleep') as mock_sleep:
                with patch.object(daemon, 'task_queue') as mock_queue:
                    ...
                    mock_check_prs.return_value = [
                        {'number': 123, 'title': 'Test PR', 'head': {'ref': 'self-improvement/test'}}
                    ]
    ...
    def test_pr_merge_queues_next_task(self, daemon):
        ...
        with patch.object(daemon, '_check_open_prs') as mock_check_prs:
            with patch.object(daemon, '_queue_next_improvement_task') as mock_queue_task:
                with patch.object(daemon, '_interruptible_sleep') as mock_sleep:
                    with patch.object(daemon.task_queue, 'get_next_task') as mock_get_task:
                        with patch.object(daemon.task_queue, 'count_pending') as mock_count_pending:
                            with patch.object(daemon.task_queue, 'count_running') as mock_count_running:
                                ...
                                with patch.object(daemon.resource_manager, 'can_accept_task') as mock_resource:
        ...
        with patch('subprocess.run') as mock_run:
            with patch('requests.get') as mock_requests_get:
                with patch.dict('os.environ', {'GITHUB_TOKEN': 'test_token_123'}):
                    ...
                    assert call_kwargs['headers']['Authorization'] == 'token test_token_123'
...
class TestNextTaskQueueing:
    ...
    def test_queue_next_improvement_task_with_todos(self, daemon):
        ...
        with patch.object(daemon.modes[TaskType.SELF_IMPROVEMENT.value], '_find_todos') as mock_find_todos:
