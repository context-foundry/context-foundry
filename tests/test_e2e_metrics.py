...
class TestE2EMetrics:
    ...
    def test_collect_from_log_file(self):
        ...
        log_file.write(
            '{"id": "msg_1", "usage": {"input_tokens": 1000, "output_tokens": 500}}\n'
        )
        log_file.write(
            '{"id": "msg_2", "usage": {"input_tokens": 2000, "output_tokens": 800}}\n'
        )
        log_file.write(
            '{"id": "msg_3", "usage": {"input_tokens": 1500, "output_tokens": 600, "cache_read_input_tokens": 500}}\n'
        )
        log_file.close()
        ...
        expected_cost = (
            (1000 / 1_000_000 * 3.00)
            + (500 / 1_000_000 * 15.00)  # Call 1
            + (2000 / 1_000_000 * 3.00)
            + (800 / 1_000_000 * 15.00)  # Call 2
            + (1500 / 1_000_000 * 3.00)
            + (600 / 1_000_000 * 15.00)
            + (500 / 1_000_000 * 0.30)  # Call 3
        )
    ...
    def test_multi_phase_collection(self):
        ...
        architect_log = tempfile.NamedTemporaryFile(
        ...
        self.collector.collect_from_log_file(
            Path(scout_log.name), session_id, "Scout", "claude-sonnet-4"
        )
        self.collector.collect_from_log_file(
            Path(architect_log.name), session_id, "Architect", "claude-sonnet-4"
        )
        self.collector.collect_from_log_file(
            Path(builder_log.name), session_id, "Builder", "claude-sonnet-4"
        )
    ...
    def test_phase_aggregation(self):
        ...
        for i in range(5):
            ...
            self.db.record_api_call(
                phase_id, "claude-sonnet-4", 1000, 500, 0, 0.0105, latency_ms=2500
            )
    ...
    def test_real_world_scenario(self):
        ...
        build_id = self.db.create_build(
            session_id=session_id, task="Build metrics system", mode="add_feature"
        )
        ...
        self.db.record_api_call(
            scout_id, "claude-sonnet-4", 3000, 1500, 2000, 0.033, 2800
        )
        self.db.record_api_call(
            scout_id, "claude-sonnet-4", 2000, 1000, 1500, 0.0195, 2500
        )
        ...
        self.db.record_api_call(
            architect_id, "claude-sonnet-4", 8000, 3500, 5000, 0.0915, 4100
        )
        self.db.record_api_call(
            architect_id, "claude-sonnet-4", 6000, 2800, 3000, 0.069, 3600
        )
