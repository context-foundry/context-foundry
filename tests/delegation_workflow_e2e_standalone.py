    ...
    def test_delegation_with_error_recovery(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.side_effect = [
                ...
                Mock(returncode=0, stdout='Dependency installed\nTask completed\n', stderr='')
            ]
...
class TestAsynchronousDelegationE2E:
    ...
    def test_async_delegation_lifecycle(self):
        ...
        with patch('subprocess.Popen') as mock_popen:
            ...
            mock_process.stdout.read.return_value = b'Build successful\nAll tests passed\n'
    ...
    def test_multiple_concurrent_async_tasks(self):
...
class TestMultiPhaseBuildWorkflow:
    ...
    def test_seven_phase_build_workflow(self, mock_run):
        ...
        phases = ['Scout', 'Architect', 'Builder', 'Test', 'Screenshot', 'Documentation', 'Deploy']
        results = {}
    ...
    def test_phase_failure_and_recovery(self, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(returncode=0, stdout='Architecture ready\n', stderr=''),  # Architect succeeds
            Mock(returncode=0, stdout='Build completed\n', stderr=''),  # Builder succeeds
            Mock(returncode=1, stdout='', stderr='Test phase failed: Import error\n'),  # Test fails
            Mock(returncode=0, stdout='Tests fixed and passing\n', stderr=''),  # Test retry succeeds
        ]
...
class TestDelegationEdgeCases:
    ...
    def test_delegation_empty_working_directory(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0,
            stdout='Task completed in current directory\n',
            stderr=''
        )
    ...
    def test_delegation_nonexistent_directory(self):
        ...
        result = delegate_to_claude_code(
            task='Test task',
            working_directory='/totally/nonexistent/path/12345'
        )

        # Should return error without attempting execution
        assert '❌' in result or 'error' in result.lower() or 'not exist' in result.lower()
