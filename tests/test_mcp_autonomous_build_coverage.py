...
class TestDetectExistingCodebase:
    ...
    def test_detect_javascript_project(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (tmppath / 'package.json').write_text('{"name": "test", "version": "1.0.0"}')

            result = _detect_existing_codebase(tmppath)

            assert result['has_code'] == True
            # Check for JavaScript/Node (case-insensitive)
            assert any('javascript' in lang.lower() or 'node' in lang.lower() for lang in result['languages'])
    ...
    def test_detect_nonexistent_directory(self):
        ...
        result = _detect_existing_codebase(Path('/nonexistent/path/that/does/not/exist'))
...
class TestDetectTaskIntent:
    ...
    def test_detect_fix_bug_intent(self):
        ...
        for task in task_examples:
            ...
            has_fix_keywords = any(word in task_lower for word in ["fix", "bug", "issue", "error", "broken", "repair"])
    ...
    def test_detect_add_feature_intent(self):
        ...
        for task in task_examples:
            ...
            has_feature_keywords = any(word in task_lower for word in ["add", "enhance", "improve", "implement", "create feature", "new feature"])
            assert has_feature_keywords, f"Task '{task}' should contain add/feature keywords"
...
class TestAutonomousBuildAndDeployEdgeCases:
    ...
    def test_baml_not_available(self, mock_get_error, mock_is_available):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert 'BAML' in result_data['error'] or 'baml' in result_data['error'].lower()
    ...
    def test_working_directory_relative_path(self, mock_intent, mock_detect, mock_baml):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('mcp_server._get_context_foundry_parent_dir', return_value=Path(tmpdir)):
                with patch('subprocess.Popen') as mock_popen:
                    ...
                    result_data = json.loads(result)
                    assert 'error' not in result_data or 'BAML' in result_data.get('error', '')
    ...
    @patch('mcp_server._detect_existing_codebase')
    @patch('mcp_server._detect_task_intent')
    def test_mode_auto_adjustment_existing_code(self, mock_intent, mock_detect, mock_baml):
