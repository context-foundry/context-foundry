...
def _identify_quality_markers(nodes: List[dict], edges: List[dict]) -> Dict[str, Any]:
    ...
    for node in nodes:
        ...
        if isinstance(data, dict):
            ...
            if any(key in str(data).lower() for key in ['retry', 'attempt', 'max_retries']):
...
def _classify_connection(source_type: str, target_type: str) -> str:
    ...
    if ('retriev' in source_lower or 'vector' in source_lower) and 'llm' in target_lower:
...
def main() -> None:
    ...
    parser.add_argument(
        '--analyze',
        type=str,
        help='Analyze a single Flowise template JSON file'
    )

    parser.add_argument(
        '--analyze-all',
        type=str,
        help='Analyze all templates in a directory'
    )
    ...
    if args.analyze:
        ...
        if result.get('success'):
            ...
            for conn in result['connection_patterns']:
                print(f"  - {conn['pattern']}: {conn['source_type']} → {conn['target_type']}")
        else:
