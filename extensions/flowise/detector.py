...
def classify_flow_type(nodes: List[dict], node_types: List[str], edges: List[dict]) -> str:
    """
    ...
    llm_count = sum(1 for nt in node_types if 'llm' in nt.lower() and 'agent' not in nt.lower())
    retriever_count = sum(1 for nt in node_types if 'retriev' in nt.lower() or 'vector' in nt.lower())
    ...
    if conversation_count > 0 or (memory_count > 0 and (llm_count > 0 or agent_count > 0)):
        return "chatbot"
...
def _extract_node_types(nodes: List[dict]) -> List[str]:
    ...
    for node in nodes:
        ...
        node_type = (
            node.get('type') or
            node.get('data', {}).get('type') or
            node.get('data', {}).get('name') or
            ''
        )
        if node_type:
