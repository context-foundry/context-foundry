...
16. [Agent Node Field Name Mismatch (Pattern #16)](#agent-node-field-name-mismatch-pattern-16)
17. [Prevention Checklist](#prevention-checklist)
...
## Agent Node Field Name Mismatch (Pattern #16)

### Symptom

**User Report** (2025-11-30):
> "I'm looking at expense-approval-workflow.json and any node that I click on gives me a white screen/error. In F12, I see: Invalid value for <svg> attribute width='1.3rem' and SyntaxError: JSON Parse error: Unexpected identifier 'object'"

**Behavior in Flowise UI**:
- ✅ Workflow imports successfully
- ✅ All nodes appear on canvas with correct positions
- ✅ All edges/connections render correctly
- ❌ Clicking ANY node causes white screen crash
- ❌ Browser console shows SVG attribute errors
- ❌ Browser console shows JSON parse errors with "object"

**Severity**: CRITICAL - All nodes uneditable, workflow completely unusable

### Root Cause

**Builder uses incorrect/outdated field names that don't match Flowise 2.2 schema**

Generated agent nodes have:
```json
{
  "data": {
    "inputs": {
      "agentModelConfig": {...},        // WRONG - should be "agentModel"
      "agentStateUpdates": [...],       // WRONG - should be "updateState" (dict, not list)
      "agentMaxIterations": 10,         // WRONG - should be "maxIterations" (string)
      "agentTimeout": 300000,           // WRONG - field doesn't exist
      "agentRetryCount": 3,             // WRONG - field doesn't exist
      "agentEnableMemory": false,       // WRONG - field doesn't exist
      "agentInterrupt": "never"         // WRONG - field doesn't exist
    }
  }
}
```

Working agent nodes have:
```json
{
  "data": {
    "inputs": {
      "agentModel": {...},              // CORRECT
      "updateState": {...},             // CORRECT (dict format)
      "maxIterations": "10",            // CORRECT (string)
      "enableAgentMessages": true,      // CORRECT
      "summarizeAfter": 5,              // CORRECT
      "isLoop": false,                  // CORRECT
      "agentName": "..."                // CORRECT
    }
  }
}
...
- **Severity**: CRITICAL - Generated flows are completely unusable
- **Frequency**: High for new Flowise builds
- **User Experience**: Appears to import successfully but crashes on interaction
- **Detection Difficulty**: High - JSON is valid, structure looks correct, fails only at runtime

### Fix Applied

**Field name conversion script**:
...
for node in data['nodes']:
    if node['data'].get('type') == 'Agent':
        ...
        # Rename fields to correct names
        if 'agentModelConfig' in inputs:
            inputs['agentModel'] = inputs.pop('agentModelConfig')
        if 'agentStateUpdates' in inputs:
            # Convert list format to dict format
            updates = inputs.pop('agentStateUpdates')
            inputs['updateState'] = {u['key']: u['value'] for u in updates}
        if 'agentMaxIterations' in inputs:
            inputs['maxIterations'] = str(inputs.pop('agentMaxIterations'))

        # Remove fields that don't exist in Flowise schema
        for key in ['agentTimeout', 'agentRetryCount', 'agentEnableMemory', 'agentInterrupt']:
            inputs.pop(key, None)

        # Add required fields
        inputs.setdefault('enableAgentMessages', True)
        inputs.setdefault('summarizeAfter', 5)
        inputs.setdefault('isLoop', False)
        inputs.setdefault('agentName', node['data'].get('label', 'Agent'))
```

### Prevention

1. **Builder MUST use working examples as templates**
   - Read from `extensions/flowise/examples/{pattern-type}/` FIRST
   - Copy field names exactly from working flows
   - Never invent field names

2. **Validate generated JSON against working flow**
   - Compare field names in `inputs` object
   - Check data types match (string vs int, dict vs list)
   - Verify no extra fields that don't exist in working examples

3. **Add to Builder prompt**:
   ```
   CRITICAL: Agent node field names must match exactly:
   - agentModel (NOT agentModelConfig)
   - updateState (dict, NOT agentStateUpdates list)
   - maxIterations (string, NOT agentMaxIterations int)
   - DO NOT ADD: agentTimeout, agentRetryCount, agentEnableMemory, agentInterrupt
   - MUST ADD: enableAgentMessages, summarizeAfter, isLoop, agentName
   ```

**Pattern ID**: flowise-agent-field-mismatch-001
...
|---------|------|---------|
| 2.2 | 2025-11-30 | Added "Agent Node Field Name Mismatch" (Pattern #16) - CRITICAL UI crash when clicking nodes due to incorrect field names (agentModelConfig vs agentModel, agentStateUpdates vs updateState). From flowise-expense-approval-v2 build |
