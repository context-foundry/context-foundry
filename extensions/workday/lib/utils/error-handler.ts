...
/**
 * Get user-friendly error message based on status and code
 * @param status - HTTP status code
 * @param code - Error code
 * @param originalMessage - Original error message
 * @returns User-friendly error message
 */
function getUserFriendlyMessage(status?: number, code?: string, originalMessage?: string): string {
  // Rate limiting
  ...
  // Invalid request
  if (status === 400) {
    return 'Invalid request. Please try a different pattern or contact support.';
  }
 ...
 */
export function validateAPIKey(apiKey: string | undefined, prefix: string = 'sk-'): void {
  if (!apiKey) {
    throw new Error(
