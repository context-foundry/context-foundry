...
| Check | Purpose |
|-------|---------|
| `scout_explains_task_decomposition` | When bootstrap Scout creates task lines, `scout-report.md` must include a `## Task Decomposition` rationale with selected task count, candidate work units, coupling/dependency rationale, why not fewer/more tasks, and requirement mapping. |
| `task_queue_well_formed` | Evaluates `TASKS.md` / `IMPL_PLAN.md` after bootstrap Scout for parser validity, duplicate IDs/descriptions, malformed progress suffixes, scaffolding busywork, vague tasks, and likely file-by-file splitting. |
